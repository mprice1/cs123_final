...
void PolarAnimated::begin(){
    ...
    temp = PolarAnimated::makeRing(15,55);
    ...
    for (int i = 0; i < m_shapes_ring->length(); i++){
        ...
        float wtf=25;
        rope r = makeRopeLine(Vector4(s.t.x/wtf,s.t.y/wtf,-.1,1),Vector4(s2.t.x/wtf,s2.t.y/wtf,-.1,1),0.025,0.3);
        m_ropes->append(r);
    }

    m_engine->camera_.eye.y += 1;
...
//    glPopMatrix();
    ...
    for(int i = 0; i < m_shapes->length(); i++){
        ...
        double temp = m_framecount / 15 - s.polr / 10 - 5;
        temp = MAX(temp, .1);
